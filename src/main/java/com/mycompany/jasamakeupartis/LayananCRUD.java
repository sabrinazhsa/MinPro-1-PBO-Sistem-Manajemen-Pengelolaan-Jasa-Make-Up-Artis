...
public class LayananCRUD {
    ...
    for (int i = 0; i < daftarPesanan.size(); i++) {
        ...
        System.out.println("   Tanggal     : " + p.gettanggalPesanan());
    ...
    public PemesananLayananMUA cariPemesanan(String idPesananCari) {
        if (idPesananCari == null) {
            return null;
        }
        for (PemesananLayananMUA p : daftarPesanan) {
            if (p.getidPesanan().equals(idPesananCari)) {
                return p;
            }
        }
        return null;
    ...
    public void updatePemesanan(String idPesananCari, String statusBaru, double hargaBaru) {
        PemesananLayananMUA p = cariPemesanan(idPesananCari);

        if (p == null) {
            System.out.println("Pesanan dengan ID [" + idPesananCari + "] tidak ditemukan.");
            return;
        }
        ...
        System.out.println("Data pesanan [" + idPesananCari + "] berhasil diperbarui!");
    }
